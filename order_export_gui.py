...
APP_VERSION = "13.23"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
SHOP_GOODS_WRITE_URL   = f"{BASE_URL}/goods/goods_write.asp"
...
def _fetch_shop_product_tax_map(driver, log=None) -> dict:
    ...
    try:
        ...
        page = 1
        while True:
            url = SHOP_GOODS_LIST_URL if page == 1 else f"{SHOP_GOODS_LIST_URL}?page={page}"
            driver.get(url)
            _time.sleep(0.8)
            ...
            for a in soup.find_all("a", href=True):
                href = a["href"]
                m = _re.search(r"goods_write\.asp\?(?:.*?&)?p_seq=(\d+)", href)
                if not m:
                    continue
                p_seq = m.group(1)
        ...
        for p_seq, name in products:
            detail_url = f"{SHOP_GOODS_WRITE_URL}?p_seq={p_seq}"
            try:
...
def run_shop_order_job(settings: dict, progress_callback=None, log_callback=None):
    ...
    try:
        ...
        # --- 상품 면세/과세 맵 빌드 (캐시 우선, 없거나 update_goods=True면 신규 수집) ---
        update_goods = settings.get("update_goods", False)
        _cached_goods, _cached_meta = _load_goods_tax_cache()
        if update_goods or not _cached_goods:
            if not _cached_goods:
                log("상품 캐시 없음 → 자동 다운로드 시작...")
            else:
                log(f"상품 업데이트 확인 후 반영 → 새로 다운로드 (기존 {len(_cached_goods)}개)...")
...
def launch_gui():
    ...
    sp_update_goods_var = tk.BooleanVar(value=False)
    ttk.Checkbutton(sp_goods_lf, text="업데이트 확인 후 반영",
                    variable=sp_update_goods_var).grid(row=0, column=1, sticky="w", padx=(12, 8))
    ...
    def _open_goods_dialog():
        ...
        dlg_update_btn = ttk.Button(btn_f, text="다운로드 / 업데이트")
    ...
    def on_sp_run():
        ...
        sp_settings = {
            ...
            "update_goods": sp_update_goods_var.get(),
        }

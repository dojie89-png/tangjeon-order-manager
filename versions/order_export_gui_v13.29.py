...
APP_VERSION = "13.29"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
def _extract_product_names_for_tax(merged: dict) -> list:
    ...
    if 주문상품:
        names = []
        for part in 주문상품.split(" / "):
            part = part.strip()
            # "제품명: 옵션" 또는 "제품명: 옵션" (콜론 앞이 상품명)
            name = part.split(":")[0].strip() if ":" in part else part
            if name:
                names.append(name)
        if names:
            return names
    # 폴백: 상품명_목록 (목록 페이지에서 가져온 단일 상품명)
...
def _fetch_shop_product_tax_map(driver, log=None) -> dict:
    ...
    try:
        ...
        # --- 상세 페이지에서 처방비용 파싱 ---
        # goods_view.asp는 조회 전용이라 가격 필드가 없음 → 편집 폼(goods_write.asp)을 사용
        _write_url = f"{BASE_URL}/goods/goods_write.asp"
        for p_seq, name in products:
            detail_url = f"{_write_url}?p_seq={p_seq}"
            try:
...
def run_shop_order_job(settings: dict, progress_callback=None, log_callback=None):
    ...
    try:
        ...
        if _cached_goods:
            ...
            product_tax_map = _fetch_shop_product_tax_map(driver, log=log)
            if product_tax_map:
        ...
        # --- 상품별 면세/과세 열 동적 전개 ---
        # 전체 주문 중 가장 많은 품목 수 파악 → 상품1_*, 상품2_*, ... 열 생성
        ...
        _excel_all_cols = _SHOP_ORDER_EXCEL_COLS + _tax_col_names
...
def launch_gui():
    ...
    def _open_goods_dialog():
        ...
        # 버튼 행 1: 상품 관리
        btn_f = ttk.Frame(dlg)
        btn_f.pack(fill="x", padx=8, pady=(0, 2))
        ...
        # 버튼 행 2: GitHub 동기화
        btn_f2 = ttk.Frame(dlg)
        btn_f2.pack(fill="x", padx=8, pady=(0, 8))
        dlg_gh_sync_btn = ttk.Button(btn_f2, text="GitHub 동기화")
        dlg_gh_sync_btn.pack(side="left", padx=(0, 4))
        dlg_gh_deploy_btn = ttk.Button(btn_f2, text="GitHub 배포 (관리자)")
        dlg_gh_deploy_btn.pack(side="left", padx=(0, 4))
        ttk.Button(btn_f2, text="닫기", command=dlg.destroy).pack(side="right")
        ...
        # ── GitHub 동기화 (다운로드) ──
        def _do_gh_sync():
            dlg_gh_sync_btn.config(state="disabled", text="동기화 중...")
            dlg_log_var.set("GitHub 상품 캐시 동기화 중...")

            def _t():
                msg = _sync_cache_from_github(
                    log=lambda m: root.after(0, lambda _m=m: dlg_log_var.set(_m))
                )
                root.after(0, lambda: dlg_gh_sync_btn.config(state="normal", text="GitHub 동기화"))
                root.after(0, _dlg_refresh)
                if "업데이트 완료" in msg:
                    root.after(0, lambda: messagebox.showinfo("동기화 완료", msg, parent=dlg))

            threading.Thread(target=_t, daemon=True).start()

        dlg_gh_sync_btn.config(command=_do_gh_sync)
        ...
        def _do_gh_deploy():
            ...
            if token:
                use_saved = messagebox.askyesno(
                    "GitHub PAT",
                    "저장된 GitHub Personal Access Token을 사용할까요?\n\n[아니오]를 누르면 새 토큰을 입력합니다.",
                    parent=dlg,
                )
                if not use_saved:
                    token = ""
            ...
            dlg_gh_deploy_btn.config(state="disabled", text="배포 중...")
            dlg_log_var.set("GitHub 배포 중...")
            ...
            def _t():
                ...
                root.after(0, lambda: dlg_gh_deploy_btn.config(state="normal", text="GitHub 배포 (관리자)"))
                if "완료" in msg:
                    root.after(0, lambda: messagebox.showinfo("배포 완료", msg, parent=dlg))
                else:
                    root.after(0, lambda: messagebox.showerror("배포 실패", msg, parent=dlg))

            threading.Thread(target=_t, daemon=True).start()

        dlg_gh_deploy_btn.config(command=_do_gh_deploy)

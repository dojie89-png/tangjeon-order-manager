...
APP_VERSION = "13.20"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
def run_shop_order_job(settings: dict, progress_callback=None, log_callback=None):
    ...
    try:
        ...
        log("상품 면세/과세 정보 수집 시작...")
        product_tax_map = _fetch_shop_product_tax_map(driver, log=log)
        log(f"상품 면세/과세 정보: {len(product_tax_map)}개 항목 완료")
        update_progress(15, "상품 정보 수집 완료")
        ...
        for idx, row in enumerate(all_list_rows):
            ...
            # 매칭 우선순위: 상품명_목록 → 주문상품
            _prod_key = (
                clean_text(merged.get("상품명_목록", "") or "")
                or clean_text(merged.get("주문상품", "") or "")
            )
            _tax_info = product_tax_map.get(_prod_key, {})
            merged["상품_공급가액"] = _tax_info.get("공급가액", "")
            merged["상품_부가세"]   = _tax_info.get("부가세", "")
            merged["상품_면세금액"] = _tax_info.get("면세금액", "")
            merged["과세구분"]      = _tax_info.get("과세구분", "")
...
def launch_gui():
    ...
    sp_out_frame.grid(row=5, column=0, sticky="ew", pady=(0, 4))
    ...
    sp_status_label.grid(row=6, column=0, sticky="w", pady=(4, 2))
    ...
    sp_progress.grid(row=7, column=0, sticky="ew", pady=(0, 6))

    # ── 버튼 행 ──
    sp_btn_frame = ttk.Frame(tab4)
    sp_btn_frame.grid(row=8, column=0, sticky="ew", pady=(4, 0))
    ...
    sp_log_text.grid(row=9, column=0, sticky="nsew", pady=(8, 0))
    sp_log_scroll = ttk.Scrollbar(tab4, orient="vertical", command=sp_log_text.yview)
    sp_log_scroll.grid(row=9, column=1, sticky="ns")

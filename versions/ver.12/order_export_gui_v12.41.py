...
APP_VERSION = "12.41"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
def is_bulk_delivery(delivery_type: str, pack_count,
    ...
    """고래한방 + 한의원으로 택배 + (팩수<30 OR 세종) → 벌크 대상"""
    ...
    if packs < BULK_MAX_PACKS:
        return True
    # 30팩 이상이어도 세종은 벌크 포함 (별도 박스 소량)
    return "세종" in clean_text(combined_address)
...
def print_page(print_driver, url: str):
    ...
    try:
        ...
        if settings.get("save_bulk_excel") and master_results:
            ...
            if bulk_판암 or bulk_관저:
                ...
                print(f"벌크 내역 저장: {bulk_path} (판암 {len(bulk_판암)}건, 관저 {len(bulk_관저)}건)")
            else:
                print("벌크 내역 없음 (판암·관저)")
        ...
        if print_queue and print_driver:
            ...
            for _pr_idx, pj in enumerate(print_queue, 1):
                ...
                try:
                    print_page(print_driver, pj["url"])
                    print(f"{kl} 출력: {pj['hospital']}\\{pj['label']}")
                except Exception as e:
    ...
    for branch in GORAE_BRANCHES + ["기타"]:
        ...
        for gi, group in enumerate(groups, start=1):
            ...
            rows.append([
                ...
                1,                                                    # M 박스수량
                "",                                                   # N 배송메시지
                sender_info.get("name", f"고래한방병원_{branch}"),    # O 보내는분성명
                sender_info.get("phone", ""),                         # P 보내는분전화번호
                sender_info.get("address", ""),                       # Q 보내는분주소
            ])
    return pd.DataFrame(rows, columns=_CJ_COLUMNS)
...
def launch_gui():
    ...
    ttk.Label(filter_lf, text="목록 선택 or 직접 입력 · 회원명 부분 일치 · 비우면 전체",

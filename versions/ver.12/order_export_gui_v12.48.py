...
APP_VERSION = "12.48"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
def run_job(settings: dict, progress_callback=None):
    ...
    try:
        ...
        member_filter = clean_text(settings.get("member_filter", "") or "")
        patient_filter = clean_text(settings.get("patient_filter", "") or "")
        ...
        patient_keywords = [clean_text(p) for p in re.split(r"[,，]", patient_filter) if clean_text(p)]
            ...
            for page_no in range(1, effective_page_limit + 1):
                ...
                for idx, item in enumerate(detail_rows, start=1):
                    ...
                    if member_filter:
                        ...
                        if member_filter not in _member:
                            print(f"    -> 제외: 주문자명 필터 '{member_filter}' 미일치 ({_member})")
                            continue
                    ...
                    if patient_keywords:
                        ...
                        if not any(kw in patient_haystack for kw in patient_keywords):
                            print(f"    -> 제외: 환자명 필터 '{patient_filter}' 미일치 ({patient_haystack})")
                            continue
...
def launch_gui():
    ...
    member_filter_var   = tk.StringVar(value="")
    patient_filter_var  = tk.StringVar(value="")
    ...
    ttk.Label(filter_lf, text="주문자명").grid(row=1, column=0, sticky="w", padx=(0, 8), pady=(6, 0))
    member_entry = tk.Entry(filter_lf, textvariable=member_filter_var, **_ENTRY_KW)
    member_entry.grid(row=1, column=1, sticky="ew", pady=(6, 0), ipady=2)
    ttk.Button(filter_lf, text="지우기",
               command=lambda: member_filter_var.set("")).grid(row=1, column=2, padx=(4, 0), pady=(6, 0))

    ttk.Label(filter_lf, text="환자명(복용자)").grid(row=2, column=0, sticky="w", padx=(0, 8), pady=(6, 0))
    patient_entry = tk.Entry(filter_lf, textvariable=patient_filter_var, **_ENTRY_KW)
    patient_entry.grid(row=2, column=1, sticky="ew", pady=(6, 0), ipady=2)
    ttk.Button(filter_lf, text="지우기",
               command=lambda: patient_filter_var.set("")).grid(row=2, column=2, padx=(4, 0), pady=(6, 0))

    ttk.Label(filter_lf, text="목록 선택 or 직접 입력 · 주문자명 부분 일치 · 환자명은 콤마로 여러 명 입력 가능 · 비우면 전체",
    ...
    save_cj_excel_var = tk.BooleanVar(value=False)
    save_label_excel_var = tk.BooleanVar(value=False)
    save_decoction_pdf_var = tk.BooleanVar(value=True)
    save_dispense_pdf_var = tk.BooleanVar(value=True)
    save_dosage_text_pdf_var = tk.BooleanVar(value=True)
    save_dosage_attachment_var = tk.BooleanVar(value=True)
    ...
    auto_change_status_var = tk.BooleanVar(value=False)
    auto_cancel_status_var = tk.BooleanVar(value=False)
    ...
    def on_run():
        ...
        settings = {
            ...
            "member_filter": member_filter_var.get().strip(),
            "patient_filter": patient_filter_var.get().strip(),

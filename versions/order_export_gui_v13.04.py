...
APP_VERSION = "13.04"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
def launch_gui():
    ...
    def on_sp_run():
        ...
        # 날짜 값 읽기: YYYY-MM-DD → YY/MM/DD (사이트 포맷)
        def _to_site_date(s):
            s = s.strip()
            import re as _re2
            ...
            return s  # 이미 다른 포맷이면 그대로

        start_raw = _to_site_date(_ph_get(sp_start_entry))
        end_raw   = _to_site_date(_ph_get(sp_end_entry))

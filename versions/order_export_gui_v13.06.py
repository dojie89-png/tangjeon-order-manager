...
APP_VERSION = "13.06"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
def run_shop_order_job(settings: dict, progress_callback=None, log_callback=None):
    ...
    try:
        ...
        # --- 목록 수집 ---
        # 폼이 POST 방식이므로 JS로 폼 필드를 채우고 submit
        def _submit_list_form(status_code, page_no):
            ...
            driver.execute_script(f"""
                ...
                if (f.page) f.page.value = '{page_no}';
            """)
        ...
        for status_code in status_codes:
            ...
            while page <= max_pages:
                if _cancelled():
                    log("취소 요청 → 목록 수집 중단")
                    break
                _submit_list_form(status_code, page)

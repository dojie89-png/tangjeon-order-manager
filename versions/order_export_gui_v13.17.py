...
APP_VERSION = "13.17"  # 버전 관리: 소수점 = 기능추가/버그수정, 정수 = 대규모 개편
...
def export_label_excel(xlsx_path: str):
    ...
    ochang_mask = pd.Series([False] * len(df), index=df.index)
    for _oc in ['한의원명', '보내는분', '보내는분_주소', '회원명']:
        if _oc in df.columns:
            ochang_mask |= df[_oc].astype(str).str.contains('오창', na=False)
...
def build_cj_upload_df(master_results: list, pdf_jobs: list) -> pd.DataFrame:
    ...
    def should_skip_for_cj(row) -> bool:
        ...
        if "입원" in clean_text(row.get("조제지시사항", "") or ""):
            for _f in ["한의원명", "보내는분", "보내는분_주소", "회원명"]:
